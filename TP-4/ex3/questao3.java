...
class questao3 {
    ...
    public static void main(String[] args) {
        ...
        if (f.exists() && !f.isDirectory()) {
            Arq.openRead(arqLerPc, charset);
            pcOuVerde = 1;
        } else {
            Arq.openRead(arqLerVerde, charset);
        }
        ...
        int tamanhoFila = 0;
        int avgAvgPiti = 0;
        Double avgPTotal = 0.00;
        Double avgAvgPT = 0.00;
        String input;
        String respPT = "";
        // double initIn;
        if (pcOuVerde == 1) {
            Arq.openRead("tmp/inPub.txt", charset);
            input = Arq.readLine();
            ...
            while (true) {
                ...
                if (tamanhoFila < 5) {
                    ...
                    avgPTotal -= respostas.removerIni().getAvgPlaytime();
                    respostas.inserirFim(games.pesquisa(idInput, tamArq));
                }
                avgPTotal += games.pesquisa(idInput, tamArq).getAvgPlaytime();
                avgAvgPT = avgPTotal / tamanhoFila;
                avgAvgPiti = (int) Math.round(avgAvgPT);
                respPT = respPT.concat(avgAvgPiti + "\n");
                input = Arq.readLine();
            }
        } else {
            input = MyIO.readLine();
        ...
        if (pcOuVerde == 1) {
            for (int i = 0; i < nDeOp; i++) {
                input = Arq.readLine();
                respPT = respPT.concat(input + "\n");
                if (input.charAt(0) == 'I') {
                    String subStrTemplate = input.substring(2);
                    int idInput = Integer.parseInt(subStrTemplate);
                    if (tamanhoFila < 5) {
                        respostas.inserirFim(games.pesquisa(idInput, tamArq));
                        tamanhoFila++;
                    } else {
                    ...
                    respPT = respPT.concat("(R) " + respostas.removerIni().getName()+ "\n");
                }
            }
            Arq.close();
            Arq.openWrite("tmp/out.txt", charset);
            Arq.print(respPT);
            respostas.printResp(respPT, pcOuVerde);
        } else {
            for (int i = 0; i < nDeOp; i++) {
                input = MyIO.readLine();
                MyIO.println(input);
                if (input.charAt(0) == 'I') {
                    ...
                    MyIO.println("(R) " + respostas.removerIni().getName());
                }
            }
            respostas.printResp(respPT, pcOuVerde);
        }
        ...
        // if (pcOuVerde == 1) {
        // Arq.openWrite("tmp/matricula_mergesort.txt", charset);
        // Arq.print("762723\t");
        // Arq.print("n(n-1)/2\t");
        // Arq.print("n\t");
