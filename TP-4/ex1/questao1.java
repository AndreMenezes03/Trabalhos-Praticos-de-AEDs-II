...
class Lista {
    ...
    public void ordenarJojo(int e, int tamArq) {
        ...
        while (esq <= dir) {
            while (array[esq].getAppId() < pivo.getAppId())
                esq++;
            while (array[dir].getAppId() > pivo.getAppId())
                dir--;
            if (esq <= dir) {
                tempG = array[esq];
                array[esq] = array[dir];
...
class Celula {
    ...
    public Celula(Game joguinho, Celula proxCelula) {
        jogo = joguinho;
        prox = proxCelula;
    }

    public Celula(Game joguinho) {
        jogo = joguinho;
        prox = null;
    }
...
class ListaD {
    ...
    public void inserirIni(Game joguinho) {
        Celula aux = primeira.prox;
        primeira.prox = new Celula(joguinho, aux);
        if (aux == null)
            ultima = primeira.prox;
        qtd++;
    }

    public void inserirFim(Game joguinho) {
        ultima.prox = new Celula(joguinho);
        ultima = ultima.prox;
        qtd++;
    }

    public void inserirPos(Game joguinho, int pos) {
        Celula celulAux = primeira;
        Celula bolso;
        int i = 0;
        if (pos >= 1 && primeira != ultima && pos <= qtd) {
            while (i < pos) {
                celulAux = celulAux.prox;
                i++;
            }
            bolso = new Celula(joguinho, celulAux.prox);
            celulAux.prox = bolso;
        }
    }

    public Game removerIni() {
        if (primeira != ultima) {
            Celula aux = primeira.prox;
    ...
    public Game remover(int posicao) {
        if (posicao >= 1 && primeira != ultima && posicao <= qtd) {
            int i = 0;
            Celula aux = primeira;
            while (i < posicao) {
                aux = aux.prox;
    ...
    public void printResp(String removidos, int local) {
        String[] tirados = removidos.split("§");
        Celula print = primeira;
        int numRemovidos = tirados.length;
        int numPrintados = 0;
        if (local == 1) {
            Arq.openWrite("tmp/out.txt", "UTF-8");
            for (int i = 0; i < numRemovidos; i++) {
                Arq.println("(R) " + tirados[i]);
            }
            while (print.prox != null) {
                Arq.println("[" + numPrintados + "] " + print.prox.jogo.printResp());
                print = print.prox;
                numPrintados++;
            }
            Arq.close();
            ...
            for (int i = 0; i < numRemovidos; i++) {
                MyIO.println("(R) " + tirados[i]);
            }
            while (print.prox != null) {
                MyIO.println("[" + numPrintados + "] " + print.prox.jogo.printResp());
                print = print.prox;
                numPrintados++;
            }
        }
...
class questao1 {
    ...
    public static void main(String[] args) {
        ...
        String charsetMyIO = "UTF-8";
        MyIO.setCharset(charsetMyIO);
        int linha;
        int pcOuVerde = 0;
        int tamArq = 4402;
        ...
        int nDeOp = Integer.parseInt(input);
        String removidos = "";
        if (pcOuVerde == 1) {
            for (int i = 0; i < nDeOp; i++) {
                input = Arq.readLine();
                if (input.charAt(0) == 'I') {
                    String subStrTemplate = input.substring(3);
                    int idInput;
                    if (input.charAt(1) == 'I' || input.charAt(1) == 'F') {
                        idInput = Integer.parseInt(subStrTemplate);
                        if (input.charAt(1) == 'I') {
                            respostas.inserirIni(games.pesquisa(idInput, tamArq));
                        } else {
                            respostas.inserirFim(games.pesquisa(idInput, tamArq));
                        }
                    } else {
                        String[] idEPos = subStrTemplate.split(" ");
                        idInput = Integer.parseInt(idEPos[1]);
                        int posIns = Integer.parseInt(idEPos[0]);
                        respostas.inserirPos(games.pesquisa(idInput, tamArq), posIns);
                    }
                } else if (input.charAt(0) == 'R') {
                    if (input.charAt(1) == 'I' || input.charAt(1) == 'F') {
                        if (input.charAt(1) == 'I') {
                            removidos = removidos.concat(respostas.removerIni().getName() + "§");
                        } else {
                            removidos = removidos.concat(respostas.removerFim().getName() + "§");
                        }
                    } else {
                        String subStrTemplate = input.substring(3);
                        int posRem = Integer.parseInt(subStrTemplate);
                        removidos = removidos.concat(respostas.remover(posRem).getName() + "§");
                    }
                }
            }
            Arq.close();
        ...
        respostas.printResp(removidos, pcOuVerde);
        // double fimIn = System.currentTimeMillis();
        // double tempoInput = fimIn - initIn;
        // if (pcOuVerde == 1) {
        // Arq.openWrite("tmp/matricula_mergesort.txt", "UTF-8");
        // Arq.print("762723\t");
        // Arq.print("n(n-1)/2\t");
        // Arq.print("n\t");
