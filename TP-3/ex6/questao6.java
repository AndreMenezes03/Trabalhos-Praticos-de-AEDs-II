...
class Lista {
    ...
    public void ordenarResp(int e, int tamArq) {
        Game pivo = new Game();
        int esq = e;
        int dir = tamArq;
        pivo = array[(esq + dir) / 2];
        while (esq <= dir) {
            while (array[esq].getReleaseDate().compareTo(pivo.getReleaseDate()) < 0)
                esq++;
            while (array[dir].getReleaseDate().compareTo(pivo.getReleaseDate()) > 0)
                dir--;
            if (esq <= dir) {
                tempG = array[esq];
                array[esq] = array[dir];
    ...
    public void printResp(int vezes, int local) {
        if (local == 1) {
            ...
            for (int i = 0; i < vezes; i++) {
                String resp = array[i].printResp();
                Arq.println(resp);
            }
            Arq.close();
        } else {
            for (int i = 0; i < vezes; i++) {
                System.out.println(array[i].printResp());
            }
        }
    }
}
