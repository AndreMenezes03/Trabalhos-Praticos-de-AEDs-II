...
class Game {
    ...
    private static void pesquisa(ArrayList<Game> games, int idInput, int tam) {
        int esq = 0;
        int dir = tam - 1;
        int meio = -1;
        boolean resp = false;
        while (esq <= dir) {
            meio = (esq + dir) / 2;
            if ((idInput == games.get(meio).app_id) == true) {
                resp = true;
                esq = tam;
            } else if (idInput > games.get(meio).app_id) {
                esq = meio + 1;
            } else {
                dir = meio - 1;
            }
    ...
    public static void main(String[] args) {
        ...
        for (linha = 0; linha < tamArq - 1; linha++) {
        ...
        Collections.sort(games,Comparator.comparing(Game::getAppId));
        ...
        while (input.equals("FIM") != true) {
            ...
            pesquisa(games,idInput,games.size());
        }
    }
}